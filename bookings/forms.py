...
class ReviewForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            # Only show bookings that have ended
            # so we can't review future bookings
            self.fields["booking"].queryset = Booking.objects.filter(
                Q(owner=user) & Q(end_date__lte=datetime.datetime.now())
            )
...
class PreVisitBookingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        pre_visits_for_pet = list(
            filter(lambda booking: booking.booking_type == 0, bookings_for_pet)
        )
