...
urlpatterns = [
    ...
    path(
        "other-users-reviews",
        views.OtherUsersReviewList.as_view(),
        name="other_users_reviews",
    ),
    path("new-review", views.NewReview.as_view(), name="new_review"),
    # Found here: https://tinyurl.com/y9pdzm6k
    path(
        "reviews/update/<pk>/",
    ...
    path(
        "start-new-booking",
        views.StartNewBooking.as_view(),
        name="start_new_booking",
    ),
    path("new-pre-visit", views.NewPreVisit.as_view(), name="new_pre_visit"),
    path(
        "pre-visit/update/<pk>/",
        views.UpdatePreVisit.as_view(),
        name="update_pre_visit",
    ),
    path(
        "new-full-booking",
        views.NewFullBooking.as_view(),
        name="new_full_booking",
    ),
    path(
        "full-booking/update/<pk>/",
        views.UpdateFullBooking.as_view(),
        name="update_full_booking",
    ),
    path(
        "bookings/delete/<pk>/",
