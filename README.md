...
A quick but clear message to the user informs them of what pawcation is. 
...
The booking page, once the user is logged in, represents the main page or homepage. In the bookings page the user will at first see all their bookings (past and future) for all their pets but they will have also the option to book a new stay for the future by clicking the "new booking" button. 

### Booking Types 
There are two kinds of bookings available on the platform: `pre-visit` and `full booking`. Pre Visit bookings are mandatory for every pet and one previsit must be completed before a user can book a full booking on the platform. This is a business decision to make sure pets can be accomodated appropriately for full bookings by having a one hour pre-visit to make sure everything goes well.
